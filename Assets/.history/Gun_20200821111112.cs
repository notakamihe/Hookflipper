    ...
    void FireBullet ()
    {
        ...
        } else if (equippedBy.GetComponent<Enemy>() != null)
        {
             ...
             targetPos = new Vector3(
                 player.transform.position.x * Random.Range(0f, 2), 
                 player.transform.position.y * Random.Range(0f, 2), 
                 player.transform.position.z * Random.Range(0f, 2)
                );
             
             RaycastHit hit;

             if (Physics.Raycast(transform.position, player.transform.position - transform.position, out hit, accuracyRange))
             {
                 print(hit);
                 ApplyBulletVelocity(bullet, hit.point);
